...
class ResourceManager:
    def __init__(self, config):
        ...
        self.get_contour = LRU(self._get_contour_unbuffered, maxsize=config['buffer_size'])
    ...
    def save_contour(self,ti,polygons):
        assert 0 <= ti < self.length
        height, width = self.height,self.width
        file_path=path.join(self.mask_dir, self.names[ti]+'.txt')
        normalized_polygon={}
        for key in polygons.keys():
            normalized_polygon[key] =polygons[key]/[width,height]
        with open(file_path, "w") as file:
            for key, array in normalized_polygon.items():
                # Flatten the array and convert it to a string without brackets
                formatted_points_str = ' '.join(map(str, array.flatten()))
                # Write the string to the file followed by a newline character
                file.write(f"{key-1} {formatted_points_str}\n")
        self.invalidate(ti)
    ...
    #     else:
    #         return None
    def _get_contour_unbuffered(self, ti):
        assert 0 <= ti < self.length
        contour_path = os.path.join(self.mask_dir, self.names[ti] + '.txt')
        contours = {}
            ...
            with open(contour_path, 'r') as file:
                for line in file:
                    ...
                    if len(parts) >= 3 and len(parts) % 2 == 1:
                        ...
                        # Aggregate points for the same class_num
                        if class_num in contours:
                            contours[class_num] = np.vstack([contours[class_num], points])
                        else:
                            contours[class_num] = points
        # print(contours)
        return contours
    ...
    def invalidate(self, ti):
        ...
        self.get_contour.invalidate((ti,))
